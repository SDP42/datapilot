...
>   baseline. **Still no model selection, no classical-vs-DL comparison,
>   no CNN/LSTM/Transformer.**
>
> No hyperparameter tuning, cross-validation, model selection,
> classical-vs-DL comparison, a CNN/LSTM/Transformer architecture,
> experiment tracking, explainability, LLM usage, API, or UI exists yet.
> Phase 8 is **in progress (8.5 deep learning modeling pipeline
> integration)**; Phase 9 onward is **not started**. See
...
| `dl_engine/` | Deep learning (PyTorch) — *Phase 8.5 deep learning modeling pipeline integration done: PyTorch optional-dependency boundary + `DLTrainingConfig` (8.1); deterministic seeding/device resolution + dataset-to-tensor boundary + training loop + `DLTrainingResult` (8.2); the first architecture — `MLPArchitectureConfig` + `build_mlp()` (8.3); `evaluate_model()` + `DLEvaluationResult`, reusing Phase-7 metric semantics (8.4); `run_mlp_modeling()` + `DLModelingResult`, chaining build→train→evaluate into one deterministic run, opt-in and separate from `run_modeling_pipeline()` (8.5). No model selection or classical-vs-DL comparison yet* |
