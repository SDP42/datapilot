...
> ⚠️ **Under active development.** Phases 1–6 of the data engine are
...
> Phase 6 complete). Phase 7 (Model Development) is in progress — the
> `ModelingSpec` contract + foundation (7.1), deterministic model
> readiness + data-split planning (7.2), and deterministic model-candidate
> generation (7.3 — recommends model families only; trains, evaluates,
> and selects nothing). Model training, DL, LLM, API, and UI
> functionality are **not started**. See
