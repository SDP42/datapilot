...
| **Phase 5 — Automated Problem Understanding** (`data_engine.problem_understanding`) | 🚧 In progress — the deterministic `ProblemSpec` contract + `understand_problem` foundation (5.1) and **target identification** — `identify_target()` ranks plausible target columns from structural evidence + transparent objective name-matching, no ML/LLM (5.2). No task-type / metric / feasibility inference yet |
