...
> - **Forecasting Foundation (done)** — an additive cross-phase (5/6/7)
>   increment: a first-class `time_column` for forecasting problems
>   (`infer_task_type` resolves it; `assess_feasibility` blocks an
>   unsorted frame), and a new `FeatureEngineeringSpec.temporal` section
>   carrying lag / rolling feature **recommendations** (nothing is built).
>
