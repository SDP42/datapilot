...
>   `dl_engine.execution.run_mlp_modeling()` chains the existing
>   components into one deterministic single-model run: seed →
>   `build_mlp` → `to_tensors` (training data) → `train_model` →
>   `to_tensors` (evaluation data) → `evaluate_model` → one
>   `DLModelingResult`. Training and evaluation data are **always** two
>   separate arrays the caller supplies explicitly — this function never
>   splits data itself and never evaluates on rows it trained on.
>   `dl_engine.contracts.DLModelingResult` (new, additive) **nests** the
>   existing `DLTrainingResult` / `DLEvaluationResult` rather than
>   duplicating their fields. **Deliberately not wired into
>   `run_modeling_pipeline()`** — a separate, opt-in Phase-8 entry point;
>   `data_engine.modeling` has zero diffs from this increment; the
>   Phase-7 `ModelFamily.NEURAL` candidate is still the scikit-learn
>   baseline.
> - **Phase 8.6 — Deep Learning Model Selection (done)** — a
>   deterministic selection layer comparing multiple Phase-8 candidates
>   **against each other only**. `dl_engine.contracts.DLCandidate` pairs
>   an existing `MLPArchitectureConfig` + `DLTrainingConfig`, identified
>   by a deterministic SHA-256 config digest (never a random UUID).
>   `dl_engine.selection.select_dl_models()` calls `run_mlp_modeling`
>   **exactly once per candidate** (no retraining) and reuses the
>   **exact** Phase-7 per-task selection metric (`rmse`/minimize for
>   regression, `f1`/maximize for binary and multiclass classification —
>   `roc_auc` stays visible but never overrides `f1`). An ineligible
>   candidate (failed run, missing/non-finite metric) stays visible in
>   the ranking with `rank = None` and a reason; when none is eligible,
>   `DLSelectionResult.status = failed`. Ties break on
>   `(architecture_name, serialised training_config)` — fully
>   reproducible, never dictionary order or a random number.
>   `dl_engine.contracts.DLCandidateRank` (new, additive) **nests** the
>   existing `DLModelingResult` per candidate. **Deliberately not wired
>   into `select_model()` or `run_modeling_pipeline()`** —
>   `data_engine.modeling` has zero diffs.
> - **Phase 8.7 — Advanced Deep Learning Architecture Foundation
>   (done)** — architecture **foundations only** for three more
>   architectures, mirroring `MLPArchitectureConfig`/`build_mlp` exactly.
...
>   itself unchanged) and each validate a real architecture-specific
>   constraint (odd `kernel_size` for CNN; `dropout == 0.0` when
>   `num_layers == 1` for LSTM; `num_heads` evenly divides `d_model` for
>   Transformer). `dl_engine.cnn.build_cnn()` / `dl_engine.lstm.
>   build_lstm()` / `dl_engine.transformer.build_transformer()`: lazy
>   PyTorch import, deterministic construction, raw logits (no
>   softmax/sigmoid), and an explicit input-shape check that raises
>   rather than reshapes. **None of the three are wired into
>   `train_model`, `run_mlp_modeling`, or `select_dl_models`** — no
>   CNN/LSTM/Transformer candidate can be trained, evaluated, or selected
>   yet; that integration is explicitly deferred. **Still no
>   classical-vs-DL comparison, no experiment tracking.**
>
> No hyperparameter tuning, cross-validation, classical-vs-DL comparison,
> advanced-architecture training/evaluation/selection, experiment
> tracking, explainability, LLM usage, API, or UI exists yet. Phase 8 is
> **in progress (8.7 advanced deep learning architecture foundation)**;
...
| `dl_engine/` | Deep learning (PyTorch) — *Phase 8.7 advanced deep learning architecture foundation done: PyTorch optional-dependency boundary + `DLTrainingConfig` (8.1); deterministic seeding/device resolution + dataset-to-tensor boundary + training loop + `DLTrainingResult` (8.2); the first architecture — `MLPArchitectureConfig` + `build_mlp()` (8.3); `evaluate_model()` + `DLEvaluationResult` (8.4); `run_mlp_modeling()` + `DLModelingResult` (8.5); `select_dl_models()` + `DLCandidate`/`DLCandidateRank`/`DLSelectionResult` (8.6); CNN/LSTM/Transformer architecture contracts + lazy-PyTorch builders — `build_cnn()`/`build_lstm()`/`build_transformer()` (8.7, foundations only, no training/evaluation/selection integration). No classical-vs-DL comparison yet* |
...
| **Phase 8.5 — Deep Learning Modeling Pipeline Integration** (`dl_engine`) | ✅ Done — connects the existing Phase-8 components into one coherent single-model workflow. `dl_engine.execution.run_mlp_modeling(X_train, y_train, X_eval, y_eval, architecture, training_config)`: seed → `build_mlp` → `to_tensors` (training data) → `train_model` → `to_tensors` (evaluation data) → `evaluate_model` → one `DLModelingResult`. Pure composition — duplicates none of the underlying functions' logic. **Training and evaluation data are always two separate arrays the caller supplies explicitly** — no splitting logic lives in `dl_engine` (Phase 7's split-execution logic is private and tightly coupled to `DataSplitPlan`; reusing or duplicating it was avoided per the plan's own fallback clause). No preprocessing happens here — `X_train`/`X_eval` must already satisfy the Phase 6.5/7.4 boundary. Evaluation is never attempted after a training stage that did not complete. `dl_engine.contracts.DLModelingResult`: a new, small, additive contract that **nests** `DLTrainingResult`/`DLEvaluationResult` rather than duplicating their fields — the same pattern `ModelingSpec` uses for its own `training`/`evaluation` sections. **Deliberately not wired into `run_modeling_pipeline()`** — a separate, opt-in Phase-8 entry point reached only by explicitly importing `dl_engine`; `data_engine/modeling/*` has zero diffs from this increment, and the Phase-7.3 `ModelFamily.NEURAL` candidate is still the unchanged scikit-learn baseline. Every failure (PyTorch missing, device unavailable, invalid/incompatible data, a training or evaluation failure, an architecture/training-config task-type mismatch) is a structured result naming the stage that stopped the run — never a raw exception, never a silent retry, never a silent device fallback. Verified deterministic (byte-identical repeated results) and verified train/evaluation separation directly (a spy confirms the exact evaluation-array contents `evaluate_model` receives are the caller-supplied `X_eval`, never `X_train`). |
| **Phase 8.6 — Deep Learning Model Selection** (`dl_engine`) | ✅ Done — a deterministic selection layer comparing multiple Phase-8 candidates **against each other only** (never against a Phase-7 classical model). `dl_engine.contracts.DLCandidate`: pairs an existing `MLPArchitectureConfig` + `DLTrainingConfig` — no new configuration vocabulary; identified by a deterministic SHA-256 digest of both configs' own JSON (never a random UUID or experiment id). `dl_engine.selection.select_dl_models(X_train, y_train, X_eval, y_eval, candidates)`: calls the existing `run_mlp_modeling` **exactly once per candidate** (verified by a call-count spy — no retraining), comparing every candidate on the same shared train/evaluation split. Selection metric reused **verbatim** from `data_engine.modeling.selection` — `rmse`/minimize for regression, `f1`/maximize for binary and multiclass classification; `roc_auc` stays visible in each candidate's nested evaluation but never overrides `f1`. A candidate is eligible only when its run completed and its evaluation carries a finite selection metric; an ineligible candidate remains **visible** in `ranking` with `rank = None` and a reason — never silently dropped; when zero candidates are eligible, `DLSelectionResult.status = failed` (a documented, deliberate difference from Phase 7's `ModelSelection`, which reports `completed` even with nothing selected). Deterministic tie-break: `(architecture_name, the candidate's own serialised training_config)` — adapted from Phase 7's `(family, estimator_name)` tie-break since every Phase-8 candidate shares `family = NEURAL`; never dictionary order, an object id, or a random number. `dl_engine.contracts.DLCandidateRank`: a new, additive contract that **nests** `DLModelingResult` per candidate rather than duplicating its fields. **Deliberately not wired into `select_model()` or `run_modeling_pipeline()`** — `data_engine/modeling/*` has zero diffs. Verified deterministic (byte-identical repeated rankings and selected candidate, including a genuine tie resolved identically every run). |
| **Phase 8.7 — Advanced Deep Learning Architecture Foundation** (`dl_engine`) | ✅ Done — architecture **foundations only** (contracts + lazy-PyTorch builders) for three more architectures; **zero training/evaluation/selection integration**. `dl_engine.architectures.CNNArchitectureConfig` / `LSTMArchitectureConfig` / `TransformerArchitectureConfig`: share `MLPArchitectureConfig`'s own `task_type`/`output_dim` convention (that existing contract is byte-for-byte unchanged) via new shared validation helpers, each with a real architecture-specific constraint validated at construction — CNN requires an odd `kernel_size` (exact same-padding, no shape-drift tracking needed); LSTM requires `dropout == 0.0` when `num_layers == 1` (matching `torch.nn.LSTM`'s own constraint); Transformer requires `num_heads` to evenly divide `d_model` (matching `torch.nn.MultiheadAttention`'s own constraint). `dl_engine.cnn.build_cnn()` / `dl_engine.lstm.build_lstm()` / `dl_engine.transformer.build_transformer()`: each mirrors `build_mlp`'s lazy-PyTorch-import discipline and deterministic-construction guarantee, returns raw logits (no softmax/sigmoid) with output dimension following the same 1/2/num_classes rule, and validates its expected input tensor shape explicitly inside `forward()` — CNN expects `(batch, input_channels, sequence_length)`; LSTM and Transformer both expect `(batch, seq_len, input_size)` — raising `ValueError` on a mismatch rather than silently reshaping. **None of the three are wired into `train_model`, `run_mlp_modeling`, or `select_dl_models`** — that integration is explicitly deferred to a later increment. Verified with real PyTorch: correct output shapes for all three task types, raw-logit outputs, invalid-shape rejection, deterministic construction when seeded, and no config mutation during construction. **No classical-vs-DL comparison, no advanced-architecture training/evaluation/selection, no experiment tracking exists yet** — Phase 8 remains **in progress**, not done. |
| Deep learning — classical-vs-DL comparison, advanced-architecture training/evaluation/selection integration (Phase 8.8+) | ⛔ Not started |
